...
DIR         = "."  # your Delphes base directory (do not change if you place this script in Delphes base directory)
OUTDIR      = "Stops100TeV" # Directory where root output files will be stored (temporrarily if sending to remote machine)
SlhaFile    = "Stops100TeV/default.slha" # default slha file with path included
PythiaCMND  = "Stops100TeV/default.cmnd" # default command file for Pythia run with path included
DelphesCard = "Stops100TeV/100TeV.tcl" # Delphes card with path included
...
if not HOSTDIR.endswith("/") : 
	HOSTDIR=HOSTDIR+"/"
for Mst in range(3000,4500,500) :
	for Mgl in range(2000,Mst-499,500) :
		process=str(Mst)+"_"+str(Mgl)
		...
		PyFile=OUTDIR+process+"/"+process+".cmnd"
		WriteCMND(PyFile, NewSlhaFile)
		# Run Delphes
		call(DIR+"DelphesPythia8 "+DelphesCard+" "+PyFile+" "+RootFile,shell=True) 
